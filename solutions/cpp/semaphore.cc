...
void IncrementThread(Semaphore& semaphore, string name) {
  while (true) {
    ...
    semaphore.Acquire();
    cout << name << " has got permit, new count is " << semaphore.NumTaken() << endl;
    for (int i = 0; i < 3; ++i) {
...
int main(int argc, char* argv[]) {
  ...
  thread increment_thread1(IncrementThread, ref(semaphore), "increment_thread_1");
  thread decrement_thread1(DecrementThread, ref(semaphore), "decrement_thread_1");
  thread increment_thread2(IncrementThread, ref(semaphore), "increment_thread_2");
  thread decrement_thread2(DecrementThread, ref(semaphore), "decrement_thread_2");
