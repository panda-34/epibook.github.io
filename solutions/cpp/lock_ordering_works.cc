...
class Account {
 ...
 private:
  bool Move(Account& to, int amount) {
    ...
    {
      // Does not matter if lock1 equals lock2: since recursive_mutex locks are
...
int main(int argc, char* argv[]) {
  Account from(200);
  Account to(100);
  cout << "Initial balances = " << from.get_balance() << ' ' << to.get_balance() << endl;
  Account::Transfer(from, to, 50);
  sleep_for(milliseconds(100));
  cout << "New balances = " << from.get_balance() << ' ' << to.get_balance() << endl;
