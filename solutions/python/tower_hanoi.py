...
def compute_tower_hanoi(num_rings):
    pegs = [[] for i in range(NUM_PEGS)]
    # Initialize pegs.
    pegs[0] = list(range(num_rings, 0, -1))

    compute_tower_hanoi_steps(num_rings, pegs, 0, 1, 2)


def compute_tower_hanoi_steps(num_rings_to_move, pegs, from_peg, to_peg,
                              use_peg):
    # @exclude
    global num_steps
    # @include
    if num_rings_to_move > 0:
        compute_tower_hanoi_steps(num_rings_to_move - 1, pegs, from_peg,
                                  use_peg, to_peg)
        ...
        num_steps += 1
        # @include
        compute_tower_hanoi_steps(num_rings_to_move - 1, pegs, use_peg, to_peg,
                                  from_peg)
