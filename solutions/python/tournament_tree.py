# TournamentTree.cc bd9b3e8c6bc4755e176bbf01d16d2a77b2bf5147
# @include
class TreeNode:
...
class TournamentTree:
    ...
    def insert_helper(self, idx, item, cap):
        ...
        if left < len(self._tree):  # _tree[idx] is an internal node.
            self.insert_helper(left if self._tree[left].cap >= cap else right, item, cap)
            self._tree[idx].cap = max(self._tree[left].cap, self._tree[right].cap)
