...
def solve_sudoku(partial_assignment):
    return solve_partial_sudoku(0, 0, partial_assignment)


def solve_partial_sudoku(i, j, partial_assignment):
    ...
    if partial_assignment[i][j] != EMPTY_ENTRY:
        return solve_partial_sudoku(i + 1, j, partial_assignment)

    for val in range(1, len(partial_assignment) + 1):
        # It's substantially quicker to check if entry val with any of the constraints if we add it at (i,j) adding it, rather than adding it and then checking all constraints. The reason is that we know we are starting with a valid configuration, and the only entry which can cause a problem is entryval at (i,j).
        if valid_to_add(partial_assignment, i, j, val):
            partial_assignment[i][j] = val
            if solve_partial_sudoku(i + 1, j, partial_assignment):
                return True
...
def valid_to_add(partial_assignment, i, j, val):
    ...
    for a in range(region_size):
        for b in range(region_size):
            if val == partial_assignment[region_size * I + a][region_size * J +
                                                              b]:
                return False
