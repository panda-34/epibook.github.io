...
def nonuniform_random_number_generation(values, probabilities):
    prefix_sum_of_probabilities = [0.0] + list(
...
def main():
    ...
    # Test. Perform the nonuniform random number generation for n * k_times times and calculate the distribution of each bucket.
    k_times = 100000
    counts = collections.Counter(
        int(nonuniform_random_number_generation(T, P))
        for i in range(n * k_times))
    for i in range(n):
