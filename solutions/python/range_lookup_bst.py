...
def range_lookup_in_bst(tree, interval):
    result = []
    range_lookup_in_bst_helper(tree, interval, result)
    return result


def range_lookup_in_bst_helper(tree, interval, result):
    if tree is None:
        return

    if interval.left <= tree.data <= interval.right:
        # tree.data lies in the interval.
        range_lookup_in_bst_helper(tree.left, interval, result)
        result.append(tree.data)
        range_lookup_in_bst_helper(tree.right, interval, result)
...
def main():
    ...
    assert sorted(range_lookup_in_bst(tree, Interval(
        16, 31))) == [17, 19, 23, 29, 31]
