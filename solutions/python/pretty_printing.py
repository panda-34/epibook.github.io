# Pretty_printing.cc bd9b3e8c6bc4755e176bbf01d16d2a77b2bf5147
import sys
import random
import string


def rand_string(length):
    return ''.join(random.choice(string.ascii_lowercase) for i in range(length))


# @include
def minimum_messiness(words, line_length):
    # min_messiness[i] is the minimum messiness when placing words[0 : i].
    min_messiness = [float('inf')] * len(words)
    num_remaining_blanks = line_length - len(words[0])
    min_messiness[0] = num_remaining_blanks ** 2
    for i in range(1, len(words)):
        num_remaining_blanks = line_length - len(words[i])
        min_messiness[i] = min_messiness[i - 1] + num_remaining_blanks ** 2
        # Try adding words[i - 1], words[i - 2], ...
        for j in range(i - 1, -1, -1):
            num_remaining_blanks -= len(words[j]) + 1
            if num_remaining_blanks < 0:
                # Not enough space to add more words.
                break
            first_j_messiness = 0 if j - 1 < 0 else min_messiness[j - 1]
            current_line_messiness = num_remaining_blanks ** 2
            min_messiness[i] = min(min_messiness[i],
                                   first_j_messiness + current_line_messiness)
    return min_messiness[-1]


# @exclude


def small_test():
    assert(minimum_messiness(('aaa', 'bbb', 'c', 'd', 'ee', 'ff', 'gggggg'),
                             11) == 45)
    assert minimum_messiness(('a', 'b', 'c', 'd'), 5) == 8


def main():
    small_test()
    if len(sys.argv) == 2:
        n = int(sys.argv[1])
        L = random.randint(10, 19)
    elif len(sys.argv) == 3:
        n = int(sys.argv[1])
        L = int(sys.argv[2])
    else:
        n = random.randint(1, 30)
        L = random.randint(11, 20)
    W = [rand_string(random.randint(1, 10)) for i in range(n)]
    print(*W, end=' ')
    print(L)
    print(minimum_messiness(W, L))


if __name__ == '__main__':
    main()
