# Closest_pair_points.cc bd9b3e8c6bc4755e176bbf01d16d2a77b2bf5147
import sys
import math
import operator
import collections
...
PairOfPointsWithDistance = collections.namedtuple(
    'PairOfPointsWithDistance', ('p1', 'p2', 'distance'))
...
def find_closest_pair_points(points):
    ...
    closest_two_points_with_distance = find_closest_pair_points_in_subarray(points, 0, len(points))
    return PairOfPoints(closest_two_points_with_distance.p1, closest_two_points_with_distance.p2)


# Returns the closest two points and their distance as a tuple in points[begin : end - 1].
def find_closest_pair_points_in_subarray(points, begin, end):
    ...
    mid_ret = find_closest_pair_in_remain(remain, best_result_in_subsets.distance)
...
def main():
    for _ in range(50):
        ...
        print('num points =', n)
        points = [Point(random.randrange(10000), random.randrange(1000)) for i in range(n)]
