# Load_balancing.cc bd9b3e8c6bc4755e176bbf01d16d2a77b2bf5147
import sys
import random
...
def decide_load_balancing(user_file_size, server_num):
    ...
    while l <= r:
        ...
        is_feasible = greedy_assignment(user_file_size, server_num, m, assign_res)
        if is_feasible:
...
def greedy_assignment(user_file_size, server_num, limit, assign_res):
    server_idx = 0
    for file in user_file_size:
        while server_idx < server_num and file + assign_res[server_idx] > limit:
            server_idx += 1
