# Binary_tree_utils.h 848813e190b1b85a8e75107fe8513c3be38ad1a9
import sys
import random
...
def generate_rand_binary_tree(n, is_unique=False):
    l = []
    root = BinaryTreeNode(n if is_unique else random.randrange(sys.maxsize))
    n -= 1
    l.append((root, 'left'))
    l.append((root, 'right'))
    while n:
        x = random.randrange(len(l))
        node = BinaryTreeNode(n if is_unique else random.randrange(sys.maxsize))
