# Compare_k-th_largest_in_heap.cc bd9b3e8c6bc4755e176bbf01d16d2a77b2bf5147
import sys
...
def compare_kth_largest_heap(max_heap, k, x):
    larger_x = equal_x = 0
    larger_x, equal_x = compare_kth_largest_heap_helper(max_heap, k, x, 0, larger_x, equal_x)
    return LARGER if larger_x >= k else (EQUAL if larger_x + equal_x >= k else SMALLER)
...
def compare_kth_largest_heap_helper(max_heap, k, x, idx, larger_x, equal_x):
    ...
    if larger_x >= k or idx >= len(max_heap) or max_heap[idx] < x:
        ...
        larger_x += 1
    larger_x, equal_x = compare_kth_largest_heap_helper(max_heap, k, x, 2 * idx + 1, larger_x, equal_x)
    larger_x, equal_x = compare_kth_largest_heap_helper(max_heap, k, x, 2 * idx + 2, larger_x, equal_x)
    return larger_x, equal_x
# @exclude
...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        print('smaller' if res == SMALLER else ('equal' if res == EQUAL else 'larger'))
    else:
