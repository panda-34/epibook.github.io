# Closest_to_median.cc bd9b3e8c6bc4755e176bbf01d16d2a77b2bf5147
import sys
import random
import heapq
...
def check_ans(A, res, k):
    A.sort()
    median = A[len(A) // 2] if len(A) % 2 else 0.5 * (A[len(A) // 2 - 1] + A[len(A) // 2])
...
def random_test_fixed_n(N):
    ...
    A = [random.randrange(2*N) for i in range(N)]
    test_multiple_k(A, order)

    A = [random.randint(0, max(N // 2, 1)) for i in range(N)]
    test_multiple_k(A, order)
