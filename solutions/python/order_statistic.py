...
# @include
# The numbering starts from one, i.e., if A = [3,1,-1,2] FindKthLargest(1, A) returns 3, FindKthLargest(2, A) returns 2, FindKthLargest(3, A) returns 1, and FindKthLargest(4, A) returns -1.
def find_kth_largest(k, A):
    return find_kth(k, operator.gt, A)


# @exclude


# The numbering starts from one, i.e., if A = [3,1,-1,2] then FindKthSmallest(1, A) returns -1, FindKthSmallest(2, A) returns 1, FindKthSmallest(3, A) returns 2, and FindKthSmallest(4, A) returns 3.
def find_kth_smallest(k, A):
    return find_kth(k, operator.lt, A)


# @include
def find_kth(k, comp, A):
    left, right = 0, len(A) - 1
    while left <= right:
        # Generates a random integer in [left, right].
        pivot_idx = random.randint(left, right)
        new_pivot_idx = partition_around_pivot(left, right, pivot_idx, comp, A)
...
# Partition A[left : right] around pivot_idx, returns the new index of the pivot, new_pivot_idx, after partition. After partitioning, A[left : new_pivot_idx - 1] contains elements that are greater than the pivot, and A[new_pivot_idx + 1 : right] contains elements that are less than the pivot.
#
# Note: "less than" is defined by the comp object.
#
# Returns the new index of the pivot element after partition.
def partition_around_pivot(left, right, pivot_idx, comp, A):
