# Search_a_pair_sorted_array.cc bd9b3e8c6bc4755e176bbf01d16d2a77b2bf5147
import sys
import operator
import itertools
import random
...
def find_pair_sum_k(A, k):
    ...
    if result.index_1 == -1 and result.index_2 == -1:
        return find_pair_using_compare(A, k, operator.lt if k >= 0 else operator.ge)
    return result
...
def find_pair_using_compare(A, k, comp):
    ...
    while result.index_1 < result.index_2:
        if A[result.index_1] + A[result.index_2] == k:
            ...
            while True:
                result.index_1 += 1
                if result.index_1 >= result.index_2 or not comp(A[result.index_1], 0):
                    break
        else:
            while True:
                result.index_2 -= 1
                if result.index_1 >= result.index_2 or not comp(A[result.index_2], 0):
                    break
...
def find_pair_sum_k_pythonic(A, k):
    # iterate array in increasing order (negative numbers backwards then positive numbers forward)
    l_indices = itertools.chain(
        filter(lambda i: A[i] < 0, range(len(A) - 1, -1, -1)),
        filter(lambda i: A[i] >= 0, range(len(A))))
    # iterate array in decreasing order (positive numbers backwards then negative numbers forward)
    ...
    r_enum = zip(range(len(A) - 1, -1, -1), r_indices)
