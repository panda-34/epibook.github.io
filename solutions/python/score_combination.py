...
def num_combinations_for_final_score(final_score, individual_play_scores):
    # One way to reach 0.
    num_combinations_for_score = [[0] * (final_score + 1) for i in individual_play_scores]
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        individual_play_scores = [random.randint(1, 1000) for i in range(random.randint(1, 50))]
    elif len(sys.argv) == 2:
        k = int(sys.argv[1])
        individual_play_scores = [random.randint(1, 1000) for i in range(random.randint(1, 50))]
    else:
